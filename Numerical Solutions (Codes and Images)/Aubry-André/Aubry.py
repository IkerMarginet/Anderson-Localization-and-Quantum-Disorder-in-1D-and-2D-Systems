...
# Define the Aubry-André Hamiltonian
def aubry_andre_hamiltonian(N, λ, φ=0.0, t=1.0, α=(np.sqrt(5)-1)/2):
    """
    Construct the 1D Aubry-André model Hamiltonian with quasiperiodic potential.
    ...
    n = np.arange(N)
    potential = λ * np.cos(2 * np.pi * α * n + φ)
...
# Theoretical localization length for Aubry-André model
def theoretical_localization_length(λ, t=1.0):
    """Calculate theoretical localization length for Aubry-André model."""
    if λ == 0:
        return float('inf')
    elif λ <= 2*t:
        return float('inf')  # Extended phase
    else:
        return 1 / np.log(λ/(2*t))  # Localized phase
...
def plot_combined_lyapunov(evals_all_λ, le_all_λ, λ_vals):
    ...
    # Add critical line at λ=2
    plt.axvline(x=0, color='red', linestyle='--', alpha=0.5, label='Critical λ=2')
...
def run_simulation(N=200, t=1.0, λ_vals=[0, 0.5, 1, 1.5, 2, 2.5, 3, 4, 5, 10], num_realizations=5):
    ...
    for λ_idx, λ in enumerate(λ_vals):
        ...
        for r in range(num_realizations):
            ...
            H, potential = aubry_andre_hamiltonian(N, λ, φ, t)
            evals, evecs = eigh(H)
        ...
        plt.title(f'Quasiperiodic Potential Profile (λ={λ})')
        plt.grid(True, alpha=0.3)
        ...
        # Localization length analysis (only for λ > 2)
        if λ > 2:
        ...
        # Print results
        print(f"  Fitted localization length ξ ≈ {xi_fit if λ > 2 else 'inf'}")
        print(f"  Theoretical ξ ≈ {xi_theory if λ >= 2 else 'inf'}")
    ...
    plt.axvline(x=2, color='red', linestyle='--', alpha=0.5, label='Critical λ=2')
