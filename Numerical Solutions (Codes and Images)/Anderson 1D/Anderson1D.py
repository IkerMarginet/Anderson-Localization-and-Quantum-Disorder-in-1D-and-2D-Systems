...
# Define the Anderson Hamiltonian
def anderson_hamiltonian(N, W, t=1.0):
    """Construct the 1D Anderson model Hamiltonian with random disorder."""
...
# Level spacing statistics with Poisson and Wigner-Dyson
...
def theoretical_localization_length(W, t=1.0):
    """Calculate theoretical localization length."""
    if W == 0:
        return float('inf')
    return 96 * t**2 / (W**2 + 1e-10)
...
def plot_dos_comparison(evals, W, t=1.0):
    ...
    plt.title(f'High-Resolution DOS Comparison (W={W})')
    plt.legend()
    ...
    plt.tight_layout()
    plt.savefig(f'dos_comparison_W{W}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_3d_wavefunction(states, energies, W, filename):
    ...
    ax.set_zlabel('|ψₙ|²')
    ax.set_title(f'3D Wavefunction Amplitudes (W={W})')
    plt.tight_layout()
    plt.savefig(filename, dpi=300)
    plt.close()

def plot_combined_lyapunov(evals_all_W, le_all_W, W_vals):
    ...
    for idx, W in enumerate(W_vals):
        ...
        if W == 0:
            plt.axhline(0, color=colors[idx], linestyle='--', 
                       label=f'W={W} (Ballistic)')
        else:
            plt.scatter(evals_avg, le_avg, s=20, color=colors[idx],
                       alpha=0.7, label=f'W={W}')
    
    plt.xlabel('Energy E')
    plt.ylabel('Lyapunov Exponent γ')
    plt.title('Lyapunov Exponents for Different Disorder Strengths')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig('combined_lyapunov.png', dpi=300, bbox_inches='tight')
    plt.close()

def plot_combined_ipr(evals_all_W, iprs_all_W, W_vals):
    ...
    plt.ylabel('IPR')
    plt.yscale('log')
    plt.title('IPR for Different Disorder Strengths')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig('combined_ipr.png', dpi=300, bbox_inches='tight')
    plt.close()

def create_time_evolution_animation(H, N, W, num_frames=100, duration=10):
    ...
    ani.save(f'time_evolution_W{W}.gif', writer=writer)
    plt.close()

# Main simulation
def run_simulation(N=200, t=1.0, W_vals=[0, 0.5, 1, 2, 5, 10, 20, 50, 100, 10000], num_realizations=5):
    ...
    for W_idx, W in enumerate(W_vals):
        ...
        plt.plot(disorder, color=colors[W_idx], linewidth=2, alpha=0.8)
        plt.fill_between(range(N), disorder, color=colors[W_idx], alpha=0.2)
        plt.xlabel('Site n')
        plt.ylabel('Disorder Potential Vₙ')
        plt.title(f'Disorder Profile (W={W})')
        plt.grid(True, alpha=0.3)
        plt.savefig(f'disorder_W{W}.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.ylabel('IPR')
        plt.yscale('log')
        plt.title(f'IPR vs Energy (W={W})')
        plt.grid(True, alpha=0.3)
        plt.savefig(f'ipr_W{W}.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        # Lyapunov Exponent plot
        plt.figure(figsize=(12, 8))
        if W == 0:
            plt.text(0.5, 0.5, 'γ → ∞ (Ballistic)', 
        ...
        plt.xlabel('Energy E')
        plt.ylabel('Lyapunov Exponent γ')
        plt.title(f'Lyapunov Exponent (W={W})')
        plt.grid(True, alpha=0.3)
        plt.savefig(f'lyapunov_W{W}.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        # Level spacing statistics
        fig, ax = plt.subplots(figsize=(10, 6))
        level_spacing_statistics(evals_avg, ax, label=f'W={W}', color=colors[W_idx])
        plt.title(f'Level Spacing Distribution (W={W})')
        plt.savefig(f'level_spacing_W{W}.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        # DOS comparison
        plot_dos_comparison(evals_all, W, t)
        ...
        plt.figure(figsize=(12, 8))
        plt.scatter(x, prob, s=50, color=colors[W_idx], alpha=0.6, 
                   label='Numerical |ψ(x)|²')
        ...
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig(f'loc_length_fit_W{W}.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        # 3D Wavefunction plot
        plot_3d_wavefunction(evecs_last, evals_avg, W, f'3d_wavefunction_W{W}.png')
        
        # Create time evolution animation
        create_time_evolution_animation(H_last, N, W)
        
        # Print results
        print(f"  Fitted localization length ξ ≈ {xi_fit:.2f}")
        print(f"  Theoretical ξ ≈ {xi_theory if W != 0 else 'inf'}")
    
    # Create combined comparison plots
    plot_combined_lyapunov(evals_all_W, le_all_W, W_vals)
    plot_combined_ipr(evals_all_W, iprs_all_W, W_vals)
